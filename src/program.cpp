...
Program& Program::calculate_jumps(vector<tuple<uint64_t, uint64_t>> jump_positions, vector<Token>& tokens) {
...
Program::Program(const Program& that) : program(nullptr), bytes(that.bytes), addr_ptr(nullptr), branches({}) {
    ...
    for (unsigned i = 0; i < that.branches.size(); ++i) {
        branches.push_back(program.get() + (that.branches[i] - that.program.get()));
    }
}
