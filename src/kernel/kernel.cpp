...
auto viua::kernel::ProcessResult::resolve(unique_ptr<viua::types::Value> result) -> void {
    unique_lock<mutex> lck{result_mutex};
    value_returned = std::move(result);
    done.store(true, std::memory_order_release);
}
auto viua::kernel::ProcessResult::raise(unique_ptr<viua::types::Value> failure) -> void {
    unique_lock<mutex> lck{result_mutex};
    exception_thrown = std::move(failure);
    done.store(true, std::memory_order_release);
}
auto viua::kernel::ProcessResult::stopped() const -> bool { return done.load(std::memory_order_acquire); }
...
auto viua::kernel::ProcessResult::transfer_exception() -> unique_ptr<viua::types::Value> {
    unique_lock<mutex> lck{result_mutex};
    return std::move(exception_thrown);
}
auto viua::kernel::ProcessResult::transfer_result() -> unique_ptr<viua::types::Value> {
    unique_lock<mutex> lck{result_mutex};
...
viua::kernel::Kernel& viua::kernel::Kernel::load(unique_ptr<viua::internals::types::byte[]> bc) {
     ...
     *  viua::kernel::Kernel becomes owner of loaded bytecode - meaning it will consider itself responsible
     * for proper
...
viua::kernel::Kernel& viua::kernel::Kernel::bytes(viua::internals::types::bytecode_size sz) {
    /*  Set bytecode size, so the viua::kernel::Kernel can stop execution even if it doesn't reach HALT
     * instruction but reaches
...
viua::kernel::Kernel& viua::kernel::Kernel::mapfunction(const string& name,
...
viua::kernel::Kernel& viua::kernel::Kernel::mapblock(const string& name,
                                                     viua::internals::types::bytecode_size address) {
...
static auto is_native_module(string module) -> bool {
    ...
    if (path.size() == 0) {
        path = support::env::viua::getmodpath(try_path, "vlib", VIUAPATH);
    }
    if (path.size() == 0) {
        path = support::env::viua::getmodpath(try_path, "vlib", support::env::getpaths("VIUAAFTERPATH"));
    }
    return (path.size() > 0);
}
static auto is_foreign_module(string module) -> bool {
    ...
    if (path.size() == 0) {
        path = support::env::viua::getmodpath(module, "so", VIUAPATH);
    }
    if (path.size() == 0) {
        path = support::env::viua::getmodpath(module, "so", support::env::getpaths("VIUAAFTERPATH"));
    }
    return (path.size() > 0);
}
    ...
    for (unsigned i = 0; i < ichain.size(); ++i) {
        ...
        if (pushed.count(element)) {
            linearised_inheritance_chain.erase(
                remove(linearised_inheritance_chain.begin(), linearised_inheritance_chain.end(), element),
...
auto viua::kernel::Kernel::deleteMailbox(const viua::process::PID pid)
    ...
    unique_lock<mutex> lck(mailbox_mutex);
#if VIUA_VM_DEBUG_LOG
    cerr << "[kernel:mailbox:delete] pid = " << pid.get() << ", queued messages = " << mailboxes[pid].size()
         << endl;
#endif
    mailboxes.erase(pid);
    return --running_processes;
...
#if VIUA_VM_DEBUG_LOG
    cerr << "[kernel:receive:send] pid = " << pid.get() << ", queued messages = " << mailboxes[pid].size()
         << "+1" << endl;
#endif
    mailboxes[pid].send(std::move(message));
}
void viua::kernel::Kernel::receive(const viua::process::PID pid,
...
#if VIUA_VM_DEBUG_LOG
    cerr << "[kernel:receive:pre] pid = " << pid.get() << ", queued messages = " << message_queue.size()
         << endl;
#endif
    mailboxes[pid].receive(message_queue);
#if VIUA_VM_DEBUG_LOG
    cerr << "[kernel:receive:post] pid = " << pid.get() << ", queued messages = " << message_queue.size()
         << endl;
#endif
}
...
static auto no_of_schedulers(const char* env_name, viua::internals::types::schedulers_count default_limit)
...
int viua::kernel::Kernel::run() {
    ...
    vp_schedulers.emplace_back(this, &free_virtual_processes, &free_virtual_processes_mutex,
...
viua::kernel::Kernel::Kernel()
    : bytecode(nullptr), bytecode_size(0), executable_offset(0), return_code(0),
      vp_schedulers_limit(default_vp_schedulers_limit), ffi_schedulers_limit(default_ffi_schedulers_limit),
