...
auto assemble_op_if(Program& program, std::vector<Token> const& tokens,
        Token_index const i,
        ...
        std::tie(addrt_target, addrt_jump_type) =
            ::assembler::operands::resolve_jump(tokens.at(i + 3), marks, instruction);
        if (if_false != "\n") {
            std::tie(addrf_target, addrf_jump_type) =
                ::assembler::operands::resolve_jump(tokens.at(i + 4), marks, instruction);
        } else {
