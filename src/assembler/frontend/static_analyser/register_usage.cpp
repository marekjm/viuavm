                ...
                namespace checkers {
                    auto map_names_to_register_indexes(Register_usage_profile& register_usage_profile,
                        ...
                        for (const auto& line : ib.body) {
                            auto directive =
                                dynamic_cast<viua::assembler::frontend::parser::Directive*>(line.get());
                            if (not directive) {
                                continue;
                            }
            ...
            namespace static_analyser {
                namespace checkers {
                    auto check_register_usage_for_instruction_block_impl(
                        ...
                        InstructionsBlock const& ib, InstructionIndex i, InstructionIndex mnemonic_counter)
                        -> void {
                            ...
                            try {
                                switch (opcode) {
                                    ...
                                    case LEAVE:
                                        /*
                                         * Just return.
                                         * Since blocks are never entered independently (and only in context
                                         * of some other frame, we will use that "outer" frame to check for
                                         * unused values, etc.
                                         *
                                         * This may lead to less-than-stellar readability for some errors;
                                         * e.g. when a value is defined inside a block, but is used neither
                                         * inside it nor in the surrounding frame.
                                         */
                                        return;
                                    case IMPORT:
                                        // do nothing
...
static auto check_register_usage_for_instruction_block(const ParsedSource& ps, const InstructionsBlock& ib)
    ...
    viua::assembler::frontend::static_analyser::checkers::map_names_to_register_indexes(
        register_usage_profile, ib);
    viua::assembler::frontend::static_analyser::checkers::check_register_usage_for_instruction_block_impl(
        register_usage_profile, ps, ib, 0, static_cast<InstructionIndex>(-1));
}
