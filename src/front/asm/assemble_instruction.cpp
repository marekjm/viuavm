...
viua::internals::types::bytecode_size assemble_instruction(Program& program, viua::internals::types::bytecode_size& instruction, viua::internals::types::bytecode_size i, const vector<Token>& tokens, map<string, std::remove_reference<decltype(tokens)>::type::size_type>& marks) {
    ...
    if (tokens.at(i) == "nop") {
        ...
        if (tokens.at(i+1).str().at(0) == '*' or tokens.at(i+1).str().at(0) == '%') {
            program.optailcall(assembler::operands::getint_with_rs_type(resolveregister(tokens.at(i+1)), resolve_rs_type(tokens.at(i+2))));
        } else {
        ...
        program.opatom(assembler::operands::getint_with_rs_type(resolveregister(tokens.at(target)), resolve_rs_type(tokens.at(target+1))), tokens.at(source));
