...
tuple<bool, string> if_watchpoint_local_register_write(viua::kernel::Kernel& kernel, const State& state) {
    ...
    if (opcode == IZERO or opcode == INTEGER or opcode == IINC or opcode == IDEC or opcode == FSTORE or
        opcode == BSTORE or opcode == STRING or opcode == VEC or opcode == VINSERT or opcode == VPUSH or
