...
static auto insert_size_and_type_prefixed_bitstring(
    ...
    std::vector<uint8_t> const bit_string) -> viua::internals::types::byte*
{
...
static auto insert_string(viua::internals::types::byte* ptr,
                          std::string const& s) -> viua::internals::types::byte*
{
    for (auto const each : s) {
        *(ptr++) = static_cast<viua::internals::types::byte>(each);
    }
...
auto opstring(viua::internals::types::byte* addr_ptr, int_op reg, std::string s)
    -> viua::internals::types::byte*
{
    *(addr_ptr++) = STRING;
...
auto optext(viua::internals::types::byte* addr_ptr, int_op reg, std::string s)
    -> viua::internals::types::byte*
{
    *(addr_ptr++) = TEXT;
    addr_ptr      = insert_ri_operand(addr_ptr, reg);
    ...
    -> viua::internals::types::byte*
{
    ...
    return insert_two_ri_instruction(addr_ptr, opcode, a, b);
}
