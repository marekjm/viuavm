...
namespace viua {
    namespace cg {
        namespace lex {
            ...
            static auto match_adjacent(vector<Token> const& tokens,
                                       std::remove_reference<decltype(tokens)>::type::size_type i,
                                       vector<string> const& sequence) -> bool {
                if (i + sequence.size() >= tokens.size()) {
                    return false;
            ...
            auto reduce_token_sequence(vector<Token> input_tokens, vector<string> const sequence)
                -> vector<Token> {
            ...
            auto reduce_directive(vector<Token> input_tokens, string const directive) -> vector<Token> {
