...
class ExternalModulesTests(unittest.TestCase):
    ...
    def testHelloWorldExample(self):
        os.system('{0} -std=c++11 -I./src/include -fPIC -shared -o World.so ./sample/asm/external/World.cpp'.format((os.getenv('CXX') or 'g++')))
        runTest(self, 'hello_world.asm', "Hello World!")

    def testReturningAValue(self):
        os.system('{0} -std=c++11 -I./src/include -fPIC -c -o registerset.o ./src/cpu/registerset.cpp'.format((os.getenv('CXX') or 'g++')))
        os.system('{0} -std=c++11 -I./src/include -fPIC -c -o exception.o ./src/types/exception.cpp'.format((os.getenv('CXX') or 'g++')))
        os.system('{0} -std=c++11 -I./src/include -fPIC -shared -o math.so ./sample/asm/external/math.cpp registerset.o exception.o'.format((os.getenv('CXX') or 'g++')))
