        ...
        namespace frontend {
            namespace static_analyser {
                ...
                namespace checkers {
                    ...
                    template<typename T>
                    auto get_operand(viua::assembler::frontend::parser::Instruction const& instruction,
                                     size_t operand_index) -> T* {
                        if (operand_index >= instruction.operands.size()) {
                            using viua::cg::lex::InvalidSyntax;
                            throw InvalidSyntax{instruction.tokens.at(0), "not enough operands"}.note(
                                "when extracting operand " + std::to_string(operand_index));
                        }
                    ...
                    auto assert_type_of_register(Register_usage_profile& register_usage_profile,
                                ...
                                auto error =
                                    viua::cg::lex::TracedSyntaxError{}
                                        .append(InvalidSyntax(register_index.tokens.at(0),
                                                    ...
                                                    .note("need pointer to " + to_string(expected_type) +
                        ...
                        if (actual_type == ValueTypes::UNDEFINED) {
                            auto inferred_type =
                                (expected_type | (access_via_pointer_dereference ? ValueTypes::POINTER
                            ...
                            if (auto r = register_usage_profile.at(Register(register_index)).second;
                                ...
                                error.append(InvalidSyntax(r.inferred.second, "")
                                                 ...
                                                 .aside(r.inferred.second,
                                                        "deduced type is '" + to_string(r.value_type) + "'"));
                            }
                            throw error;
                        }
                    ...
                    auto check_for_unused_registers(Register_usage_profile const& register_usage_profile)
                        -> void;
